...
$(document).ready(function () {
	...
	var swiper = new Swiper('.content .swiper-container',{					
		//loop: true,
		slidesPerView:'auto',
	...
	$('.scrollDown').bind('click',function(){
		if($(window).width()>1000){
			$('body').animate({scrollTop:$('.moreDetails').position().top+75},500);
		}else{
			$('body').animate({scrollTop:$('.moreDetails').position().top},500);
		}
	});
	$(window).bind('scroll', function() {
	   ...
	   if($(window).width() > 1000){
			top = $(this).scrollTop();
			offsetBgTop = (-0.02*top);
	   
		   if(top > 200 && top < 1100){
			   $('.detailPage.fullContent .section.s1').css({'background-position':'50% '+offsetBgTop+'%'});
		   }else{
			   $('.detailPage.fullContent .section.s1').css({'background-position':'50% 0%'});
		   }
	   }else{
			top = $(this).scrollTop();
			offsetBgTop = (-0.01*top);
